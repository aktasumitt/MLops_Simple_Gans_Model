...
def model():
    ...
    label=torch.randint(0,5,size=(1,))
    return config,random_noise,label, load_obj(config.generator_save_path)


def test_model(model):
    """Modelin doğru çalışıp çalışmadığını test eder."""
    config,random_noise,label, model = model

    # Test verisini modele veriyoruz
    model.eval()
    generated_img = model(random_noise,label)
